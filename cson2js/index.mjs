import { __EMPTY__, jsonParse, __COMMENTS_KEY__, setComments, keys, regexp, REG_CRLF, jsonStringify, __TRUE__, __FALSE__ } from '../lib';
/* eslint-disable security/detect-unsafe-regex */
// /^['"](?:\s*[\r\n\u2028\u2029]\s*)?|\\\s*[\r\n\u2028\u2029]\s*|(?:\s*[\r\n\u2028\u2029]\s*)?['"]$/g
// prettier-ignore

var reg1 = regexp(['^[\'"](?:\\s*', REG_CRLF, '\\s*)?|\\\\\\s*', REG_CRLF, '\\s*|(?:\\s*', REG_CRLF, '\\s*)?[\'"]$'], 'g'); // /\s*[\r\n\u2028\u2029]\s*/g

var reg2 = regexp(['\\s*', REG_CRLF, '\\s*'], 'g'); // /\\\s*[\r\n\u2028\u2029]\s*|(?:[\r\n\u2028\u2029]\s*)?'''$/g
// prettier-ignore

var reg3 = regexp(['\\\\\\s*', REG_CRLF, '\\s*|(?:', REG_CRLF, "\\s*)?'''$"], 'g'); // /^'''(?:\s*?[\r\n\u2028\u2029])?/

var reg4 = regexp(["^'''(?:\\s*?", REG_CRLF, ')?'], ''); // /[\r\n\u2028\u2029]+/

var reg5 = regexp([REG_CRLF + '+'], '');
var __KEY__ = {}; // 'key'
...
var jsonStringParse = string => {
  ...
  if (first === '"' || first !== string[1]) {
    string = string.replace(reg1, __EMPTY__).replace(reg2, ' ').replace(/\\[^]/g, r => r[1] === first ? first : r);
  } else {
    string = string.replace(reg3, __EMPTY__).replace(/\\[^]/g, r => r[1] === "'" ? "'" : r);
    var arr = string.split(/(?![^\r\n\u2028\u2029])/g);

    var arr0 = (arr.shift() || __EMPTY__).replace(reg4, __EMPTY__);

    do {
      first = __EMPTY__; // prettier-ignore

      arr.every(v => !v[1] || !first && (first = v[1]) || first === v[1] || (first = __EMPTY__));
...
var jsonValueParse = v => {
  ...
  var res;
  var tmp;
  if (v === __TRUE__ || v === __FALSE__) res = v === __TRUE__; // else if (v === '-Infinity') res = -Infinity
...
var getCommentsFromArr = token0 => {
  ...
  if ((tmp0 = token0[1] === __ARRAY__) || token0[1] === __VALUE__) {
    comments = tmp0 ? token0[2][__COMMENTS_KEY__] || {} : {
      0: token0[3]
    };
  }
...
var cson2js = source => {
  var tokens = [];
  var token;
  var tokenLast = [-2, __EMPTY__, __EMPTY__, []];
  var comments = [];
  var deep = -1;
  source.replace(/('''|'|")(?:[^\\]|\\\1|\\(?!\1).)*?\1|(:)|([,{}[\]])|(#[^\r\n\u2028\u2029]*)|(\s+)|([^\r\n\u2028\u2029,'"#:{}[\]]+)/g, (_result, _string, _colon, _punct, _comment, _space, _other) => {
    ...
    if (_space) {
      ...
      if (arr.length > 1 || deep < 0) deep = (arr.pop() || __EMPTY__).length;else type = __SPACE__;
      ...
      if (tokenLast[1] !== __PUNCT__) {
        ...
        if (tokenLast[2][0] === "'" || tokenLast[2][0] === '"') tokenLast[2] = jsonParse(tokenLast[2]);
      }
    } else if (_punct) {
      type = __PUNCT__;
    } else if (_comment) {
    ...
    if (token0 = tokens[i]) {
      ...
      if (token0Type === __PUNCT__) {
        ...
        if (token0Type === __KEY__) {
          ...
          __plistComments__ = { ...(token0Value[__COMMENTS_KEY__] || {}),
            ...(token1Value[__COMMENTS_KEY__] || {})
          };
